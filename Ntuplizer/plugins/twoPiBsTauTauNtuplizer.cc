...
//===================================================================================================================
twoPiBsTauTauNtuplizer::twoPiBsTauTauNtuplizer( edm::EDGetTokenT<reco::MuonCollection>  muonToken   ,
...
//===================================================================================================================
twoPiBsTauTauNtuplizer::~twoPiBsTauTauNtuplizer( void )
{

}


bool twoPiBsTauTauNtuplizer::fillBranches( edm::Event const & event, const edm::EventSetup& iSetup ){
...
// 2015 trigger
      if(trigNames.triggerName(i).find("HLT_HIUPCSingleMuNotHFtwoPixel_SingleTrack_v")!= std::string::npos){
    ...
    for(int iii = 0; iii < numOfch; iii ++){
      ...
      for(int jjj = iii+1; jjj < numOfch; jjj ++){
      	...
      	//for(int kkk = jjj+1; kkk < numOfch; kkk ++){
      	  ...
      	  Int_t tau_charge = pf1.charge() + pf2.charge(); 
...
//      	  if(Taucand.fls3d < c_fsig) continue;
          ...
          std::vector<RefCountedKinematicParticle> allParticles;
      	  ...
      	  math::PtEtaPhiMLorentzVector tlv_tau_fit = tt1_fit + tt2_fit;
...
//	Float_t ptres3 = 999;
    ...
    nBranches_->matched_pion_deltaPt.push_back(tau3_fit.Pt()-tlvs[nnn].Pt());
	    ...
	    Bool_t isRight_ = isRight1_ && isRight2_;
...
//   std::cout << "Built " << cands.size() << " tau candidates from " << numOfch << " pion candidates" << std::endl;
...
twoPiBsTauTauNtuplizer::ECaloType

twoPiBsTauTauNtuplizer::GetTowerSubdetHad(double&eta) const
{
//  if (TMath::Abs(eta)>3. && TMath::Abs(eta)<5.2)
